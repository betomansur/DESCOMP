...
with open(outputBIN, "w+") as f:  #Abre o destino BIN
    ...
    for line in lines:    
        ...
        if ":" in line: #LABELS
            line = line.replace("\n", "")
            print("tmp(" + str(cont) + ') := ' + '"00" &' +' x"0" & ' + "'0'" + ' & x"00"; -- NOP #'+ line + "pra pular na linha " + str(cont))
            cont+=1
        ...
        #Se a linha for válida para conversão, executa
            ...
            if '@' in instrucaoLine: #Se encontrar o caractere arroba '@' 
                if noveBits == False:
                    ...
                    label, instrucaoLine, registrador = converteArroba9bits(instrucaoLine) #converte o número após o caractere Ex(JSR @14): x"9" x"0E"

                    if label == True:
                        line = 'tmp('+ str(cont)+') := ' + '"00" &' +' x"' + instrucaoLine[0] + instrucaoLine[2] + '";\t-- ' + comentarioLine + '\n'  #Formata para o arquivo BIN
                    else:
                        if registrador == True:
                            splitado = list(instrucaoLine)
                            num_reg = splitado[1] + splitado[2]
                            instrucao = splitado[0]
                           # print(num_reg)
                            for item in reg:
                                if num_reg in reg:
                                    value = reg[num_reg]

                            line = 'tmp(' + str(cont) + ') :=  ' + '"' + value + '"' + ' & ' +'x"' + instrucao +  '" & ' + "'" + splitado[9] + "' & x"+ '"' + splitado[16] + splitado[17] + '" ;\t-- ' + comentarioLine + '\n'  #Formata para o arquivo BIN
                        else:
                            line = 'tmp(' + str(cont) + ') :=  ' + '"00" & ' +'x"' + instrucaoLine + '";\t-- ' + comentarioLine + '\n'  #Formata para o arquivo BIN
                ...
                if noveBits == False:
                    ...
                    label, instrucaoLine, registrador = converteCifrao9bits(instrucaoLine) #converte o número após o caractere Ex(LDI $5): x"4" x"05"
